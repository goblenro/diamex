...
function renderProducts() {
    ...
    const itemsHtml = sortedProducts.map(product => {
        ...
        if (Array.isArray(product.offers) && product.offers.length) {
            ...
            priceDisplay = (low !== null && high !== null) ? (low === high ? `${low.toFixed(2)} ${currency}` : `${low.toFixed(2)} - ${high.toFixed(2)} ${currency}`) : '';
        ...
        const priceHtml = priceDisplay ? `<span class="product-price" style="font-weight:700; color:#333; margin-left:12px;">${priceDisplay}</span>` : '';
