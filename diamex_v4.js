...
function renderProducts() {
    ...
    const itemsHtml = sortedProducts.map(product => {
        ...
        // Dacă produsul are mai multe mărimi -> afișăm 'startingFrom' + prețul minim
		...
		const startingLabel = (hasMultipleSizes) ? `<div class="price-starting" style="position:absolute; width:100%; top:-15px; right:0; text-align:center; color:#999; font-size:0.75em;">${(translations[currentLang] && translations[currentLang].startingFrom) || 'Starting from'}</div>` : '';
        const priceHtml = priceDisplay ? `<div class="product-price" style="position: relative;">${priceDisplay}${startingLabel}</div>` : '';
        const typeAndPriceRow = `<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
            <div class="product-type">${translateCategoryLabel(product.type)}</div>
            ${priceHtml}
        </div>`;
